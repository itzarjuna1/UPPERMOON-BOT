...
from UPPERMOON.utils import AloneBin, get_channeplayCB, seconds_to_min
from UPPERMOON.utils.database import (get_cmode, is_active_chat, is_music_playing)
from UPPERMOON.utils.decorators.language import language, languageCB
from UPPERMOON.utils.inline import queue_back_markup, queue_markup
...
def get_dev_button():
    """Return only the developer button"""
    return InlineKeyboardMarkup(
        [[InlineKeyboardButton("˹ ɪɴꜰɪɴɪᴛʏ ꭙ ʙσᴛs ˼", url="https://t.me/dark_musictm")]]
    )
...
async def get_queue(client, message: Message, _):
    ...
    upl = get_dev_button()  # <-- Only developer button used
    basic[videoid] = True
                ...
                if await is_active_chat(chat_id):
                    if basic[videoid]:
                        if await is_music_playing(chat_id):
                            try:
                                await mystic.edit_reply_markup(reply_markup=get_dev_button())
                            except FloodWait:
                                pass
                        else:
                            pass
...
async def queued_tracks(client, CallbackQuery: CallbackQuery, _):
    ...
    await CallbackQuery.answer()
    basic[videoid] = False
    buttons = get_dev_button()  # <-- Only developer button
    med = InputMediaPhoto(
...
async def queue_back(client, CallbackQuery: CallbackQuery, _):
    ...
    send = _["queue_6"] if DUR == "Unknown" else _["queue_7"]
    cap = _["queue_8"].format(app.mention, title, typo, user, send)
    upl = get_dev_button()  # <-- Only developer button
    basic[videoid] = True
